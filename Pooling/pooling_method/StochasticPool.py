...
class StochasticPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if torch.cuda.is_available():
            idx = torch.randint(0, x.shape[1], size=(x.shape[0],)).type(torch.LongTensor).cuda()
            #x = torch.tensor([x[p_r][idx[p_r]] for p_r in range(x.shape[0])]).cuda()
        else:
            idx = torch.randint(0, x.shape[1], size=(x.shape[0],)).type(torch.LongTensor)

        x = x[torch.arange(0, x.shape[0]),idx]
