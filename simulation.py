...
class PoolingSimulation():
    def __init__(self, configs, train_loader, val_loader, test_loader, **kwargs):
        self.kwargs = kwargs
    ...
    def run(self, individual):
        ...
        __C = self.configs
        __C.gene = individual.gene

        if __C.pooling == 'genetic':
            sim_data['op_count'] = quadtree_op_count(individual.gene)
        elif __C.pooling == 'ea':
            sim_data['op_count'] = ea_op_count(individual.gene)
        ...
        log_path = os.path.join(__C.result_log_dir, "eval")
        if not os.path.exists(log_path):
            os.makedirs(log_path)
        log_path = os.path.join(log_path, __C.name + '.txt')

        # write the hyper parameters to log
        logfile = open(log_path, 'a+')
        logfile.write(str(__C)+'\n')
        logfile.write('Individual: '+ individual.name +'\n')
        logfile.write(individual.print()+'\n')
        logfile.close()
        ...
        for epoch in range(1, __C.epoch):
            ...
            if __C.eval_every_epoch:
                ...
                logfile.write(
                    ...
                    ', Valid Accuracy: {:.4f}'.format(val_acc) +
                    '\n'
                )
                logfile.close()
